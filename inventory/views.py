...
from inventory.forms import RegistrationForm, ProductForm,CRFQForm,ClientRegistry,ClientProfileForm,CustomAuthenticationForm,UserProfileForm,ProfileImageForm
...
def crfq(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("crfq")
    else:
        form = CRFQForm()
    context = {"title": "CRFQ", "crfq": crfq, "form": form}
...
def clear_data(request):
    ...
    if request.method == 'POST':
        ...
        return redirect("crfq")  # Redirect to a specific page after deletion
    return render(request, 'inventory/crfq.html')
